...
def parse_input(user_input):
    ...
    banned_thorns = [str(t) for t in user_input[1].split(',')]
    max_tiredness = float(user_input[2])
    tiredness_raw = [float(n) for n in user_input[3].split(',')]
    

    if len(tiredness_raw) == 1:
        tiredness_raw.append(tiredness_raw[0])
        tiredness_raw.append(tiredness_raw[0])
    elif len(tiredness_raw) == 2:
        tiredness_raw.append(0)


    tiredness_dict = {1: tiredness_raw[0], 2: tiredness_raw[1], 3: tiredness_raw[2]}
    # if len(tiredness_raw) > 1:
    #     for i, value in enumerate(tiredness_raw, start=1):
    #         tiredness_dict[i] = value

    return height, banned_thorns, max_tiredness, tiredness_dict

def get_all_paths(height, banned_thorns, max_tiredness, tiredness_dict):
    total_valid_path = 0

    def dfs(current_height, used_energy):
        nonlocal total_valid_path
        if (used_energy > max_tiredness) or (current_height != 0 and str(current_height) in banned_thorns) or (current_height > height):
            return
        if current_height == height:
            total_valid_path += 1
            return
        for step_size in range(1, 4):
            dfs(current_height + step_size, used_energy + tiredness_dict[step_size])

    dfs(0, 0)
    return total_valid_path


user_input = input('Creating a simulated hell scenario: ').split('/')
height, banned_thorns, max_tiredness, tiredness_dict = parse_input(user_input)
total_valid_path = get_all_paths(height, banned_thorns, max_tiredness, tiredness_dict)

print(f'''Height: {height}
thorn At: {banned_thorns}
Max Tiredness: {max_tiredness}
Tiredness Values: {tiredness_dict}
--------------------------------------------------
The ways to escape is/are {total_valid_path} ways''')
